...
const IconDescription ="text-white font-semibold text-[.7em] lg:text-[16px]"
...
const Destaque: React.FC = ()=>{
    ...
    const AddItem = (e: any) => {//aqui estou adicionando
        ...
        const filterElement = itens.find((objeto) => objeto.id == idDiv)//procura o elemento dentro do array com o id igual ao li que foi selecionado
    ...
    return(
        <section className="w-full h-max py-7 m-auto flex flex-col items-center gap-7">
            <div className="flex flex-col items-center gap-6">
                ...
                :
                <ul className="flex w-full flex-row px-4 lg:px-0 lg:justify-center flex-wrap gap-3 lg:gap-6">
                    {itens.map((Element: itensJson)=>(
                        <li className="relative w-45 lg:w-max" key={Element.id} id={`${Element.id}`}>
                            ...
                            {/*Contedo do hover */}
                          <div className={`id${Element.id} absolute flex flex-col opacity-0 hover:opacity-100 hover:bg-[#0000008a] items-center justify-center inset-0 h-48 lg:h-[275px] cursor-pointer`}>
                                <div className="w-full h-[110px] flex flex-col items-center justify-between pt-5">
                                    ...
                                    <ul className="flex flex-row items-center justify-between w-full px-2 lg:px-0 lg:w-[230px]">
