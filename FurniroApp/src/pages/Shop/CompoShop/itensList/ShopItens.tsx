...
const ListItens = ({page, quantIten}: {page: number | null; quantIten: number | null}) => {
    ...
    return (
        <section className="lg:px-[70px] pt-[70px]">
            {itens.length === 0 ? <img className="w-20 m-auto" src={'https://cdn.pixabay.com/animation/2023/08/11/21/18/21-18-05-265_256.gif'}/> :
            <ul className="flex w-full flex-row px-4 lg:px-0 justify-center flex-wrap gap-3 lg:gap-6">
                {itens.map((Products) => (
                    <li className="relative w-45 lg:w-max" key={Products.id} id={`${Products.id}`}>
                        ...
                        <div className="bg-[#F4F5F7] pb-8 flex flex-col gap-3">
                            ...
                            <Link key={Products.id} to={`/Productcart/${Products.id}`}>
                                <article className="w-40 lg:w-[230px] mx-auto h-[120px] flex flex-col gap-1 pt-2">
                                    <p className="text-[18px] font-semibold">{Products.titleName}</p>
                                    <p>{Products.description}</p>
                                    <div className="flex flex-col-reverse lg:flex-row lg:items-center w-full justify-between">
                            ...
                            {/*Conteudo do hover */}
                            <div className={`id${Products.id} absolute flex flex-col opacity-0 hover:opacity-100 hover:bg-[#0000008a] items-center justify-center inset-0 h-48 lg:h-75 cursor-pointer`}>
